...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(input_size=embed_size + hidden_size, #TODO: previously just embed_size
                                   hidden_size=hidden_size,
                                   bias=True)
    ...
    def encode(
        ...
        # return a tensor of shape (b, src_len, 2*h) as `enc_hiddens`.
        
        X_packed = pack_padded_sequence(X, source_lengths)
        # When bidirectional=True, output, h_n, c_n will contain a concatenation of the forward and
        # reverse hidden states at each time step in the sequence.
        # output shape = (seq_len, b, 2 * hidden_size)
        ...
        # This is c_0^{dec} in the PDF. Here b = batch size, h = hidden siz
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        assert Y.shape[0] == target_padded.shape[0]
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_unsq = dec_hidden.unsqueeze(1)  # shape: (b, 1, h)
        # print('dec_hidden_unsq.shape: ', dec_hidden_unsq.shape) # torch.Size([32, 1, 256])
        # print('enc_hiddens_proj.shape: ', enc_hiddens_proj.shape) # torch.Size([32, 21, 256])
        e_t = torch.bmm(dec_hidden_unsq, enc_hiddens_proj.transpose(1, 2))  # shape: (b, 1, src_len)
        e_t = e_t.squeeze(1)  # shape: (b, src_len)
        ...
        # ANSWER: to reduce probability to zero in softmax
        ...
        a_t = torch.bmm(alpha_t_unsqueezed, enc_hiddens) # shape (b, 1, 2h)
        a_t = a_t.squeeze(1) # shape (b, 2h)
